...
const FotoPerfil: React.FC<FotoPerfilProps> = ({ idEntidade, entidade, initialImage }) => {
    ...
    const handleImageUpload = async (uri: string) => {
        ...
        formData.append('file', {
            uri: uri,
            name: `${entidade.toLowerCase()}-${idEntidade}.jpg`, // Ajuste o nome conforme necessário
            type: 'image/jpeg', // Tipo de arquivo
        });

        try {
            const response = await fetch(`${config.IP_SERVER}/${entidade.toLowerCase()}/${idEntidade}/upload`, {
                method: 'POST',
                body: formData,
                headers: {
